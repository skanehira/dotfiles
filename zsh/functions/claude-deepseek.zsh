...
#   ccsp qwen           使うモデルを指定する (qwen / vision / v41 / glm)
...
ccsp() {
  ...
  case "$1" in
    ...
    -h|--help)
      cat <<'USAGE' >&2
使い方:
  ccsp [lan|ts] [qwen|vision|v41|glm] [claude に渡す引数...]
...
xhigh、DeepSeek-v4.1-Flash-EXL3 と GLM-5.3-Flash-EXL3 は max、他は high)。CCSP_EFFORT=<値> ccsp で上書きできるが、モデルの語彙に
...
USAGE
    ...
    status)
      ...
      echo "  effort: ${CCSP_EFFORT:-(配信モデルで決める: qwen3.8-flash-next→xhigh / DeepSeek-v4.1-Flash-EXL3→max / GLM-5.3-Flash-EXL3→max / 他→high)}"
