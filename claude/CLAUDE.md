...
- すべてのコーディングタスクについて、適切な低労力のモデルを判断で選び、サブエージェントで実行せよ
...
- 複数ファイル・複数観点にまたがる調査や、互いに独立した作業は、同一ターンで複数のサブエージェントを並列起動して fan-out する
- 直接見えているコードの修正など 1 レスポンスで完結する作業には、サブエージェントを立てず自分で行う
