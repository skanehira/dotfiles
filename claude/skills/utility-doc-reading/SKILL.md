...
description: AI が生成したドキュメントや技術文書を、前提知識が不足しているユーザが「作業を進められるレベル」で理解できるよう対話的に支援する読解支援スキル。知識プロファイル (~/.claude/knowledge-profile.md) を参照・更新しながら、未知の概念だけを理解ゴールに応じた深度で解説する。「このドキュメントを解説して」「この設計書を理解したい」「読むのを手伝って」「前提知識が足りない」「このドキュメント何が書いてあるの」「DESIGN.md を読み解きたい」など、ドキュメントの理解・読解に関するリクエストで必ず使用する。ユーザがドキュメントを提示して内容について質問してきた場合も、単発の Q&A で済まない規模なら本スキルを起動する。
