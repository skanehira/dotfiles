...
書き直したプロンプト全文と、「どの observation をなぜ適用したか」の簡潔な要約をユーザに提示したうえで、`AskUserQuestion` で確認する。

```javascript
AskUserQuestion({
  questions: [
    {
      question: "このプロンプトで作業を続行しますか?",
      header: "プロンプト確認",
      options: [
        {
          label: "このまま続行",
          description: "書き直したプロンプトを正式な作業依頼として作業を開始する"
        },
        {
          label: "修正したい",
          description: "フィードバックを踏まえて [4/6]〜[6/6] をやり直す"
        },
        {
          label: "プロンプトだけ受け取る",
          description: "作業は開始せず、書き直したプロンプトの提示のみで終了する"
        }
      ],
      multiSelect: false
    }
