# workflow-autopilot HTML レポート テンプレート参照
...
- ファイルパス: `docs/autopilot-reports/${run_id}.html`
...
<head>
<meta charset="UTF-8">
<title>autopilot report ${run_id}</title>
...
<header class="border-b pb-6" style="border-color: var(--border);">
  <h1 class="text-2xl font-semibold">workflow-autopilot report</h1>
...
<footer class="border-t pt-6 text-xs space-y-1" style="border-color: var(--border); color: var(--fg-muted);">
  ...
  <p>このレポートは <code>~/.claude/logs/workflow-autopilot/${run_id}/decisions.jsonl</code> から生成されました</p>
</footer>
```
...
autopilot レポートは「ローカルで開いて振り返る」「リポジトリにコミットして PR レビューで参照する」が主な用途で、外部に公開しないため CDN compromise の影響範囲は限定的。それでも、以下の状況では別方式を検討する:
...
- **オフラインでも見たい** → Tailwind の最小 build を `docs/autopilot-reports/_assets/tailwind.css` に置いて相対参照
...
autopilot は単純な文字列組み立てで HTML を作る (ビルドツール不要、外部 npm 依存なし)。
