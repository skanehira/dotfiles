---
name: workflow-spec
description: "対話的計画コマンド。requirements-analyzing-requirements と implementation-planning-tasks スキルを統合実行して DESIGN.md (概要)、DESIGN_DETAIL.md (詳細)、TODO.md を生成"
argument-hint: "[タスク説明]"
...
## [2/6] DESIGN.md / DESIGN_DETAIL.md 生成（requirements-analyzing-requirements）

requirements-analyzing-requirementsスキルを実行して DESIGN.md (概要) と DESIGN_DETAIL.md (詳細) の 2 ファイルを生成します。

### スキル実行

以下の情報を requirements-analyzing-requirements スキルに渡してください：
...
Skill ツールを使用して requirements-analyzing-requirements スキルを実行してください。
...
## [3/6] 設計書の深掘り（requirements-interview）

requirements-interview スキルを実行して DESIGN.md / DESIGN_DETAIL.md をブラッシュアップします。

### スキル実行

`requirements-interview` スキルを Skill ツールで実行する（引数に `docs/DESIGN.md` を渡す。スキル内部で DESIGN_DETAIL.md も併せて読み込まれる）。

requirements-interview スキルが以下を実行します：
...
### 依存関係
- requirements-analyzing-requirements スキルが必須 (DESIGN.md + DESIGN_DETAIL.md 生成)
- requirements-interview スキルが必須 (両ファイル深掘り)
