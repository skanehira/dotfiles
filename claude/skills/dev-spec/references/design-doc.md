...
<セットアップ・dev server 起動・テスト実行・lint の実行方法。E2E (Playwright) があるプロダクトはそのセットアップと実行手順もここに書く。
 issue の DoD はここに書かれた前提 (起動済みの server 等) の上で実行される。
 実装者とレビュワーはこの節を読んでから DoD・E2E を実行する。
 **worktree セットアップ手順**もここに書く: チェックアウト直後の作業ツリー (依存も .env 系も無い状態) で
 DoD を実行可能にするコマンドと、dev server のポートを外から指定する方法。
 /dev-impl はこの手順の有無で issue を並列実装してよいかを判定する (無ければ 1 件ずつになる)>
...
<実装が必ず従う、検証済みのプラットフォーム上限・ライブラリの性質・運用基準。
 「知らずに書くと動くように見えて壊れる」ものだけを、守り方とセットで 1 行ずつ書く。
 例: 「D1 は 1 クエリの束縛変数が 100 個まで。IN 句は 100 件ごとに分割して複数回引く」
     「ULID の辞書順は追記順と一致しない (同一ミリ秒内は乱数で決まる)。順序の比較に使わない」
 別節が正本を持つものは再掲せず節名で参照する (例: 時刻の基準 → 「インフラ > 定期実行」)。
 なければ「なし」>
