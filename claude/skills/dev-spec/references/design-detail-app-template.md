...
- [6. エラー戦略](#6-エラー戦略)
- [7. UX 設計 / CLI インターフェース仕様](#7-ux-設計--cli-インターフェース仕様)
- [8. テスト戦略](#8-テスト戦略)
- [9. 検証手順](#9-検証手順)
...
```bash
# webapp: [dev サーバ起動コマンド]   # 例: npm run dev → http://localhost:5173 で初期画面表示
# cli:    [ビルド + 実行コマンド]    # 例: cargo build && ./target/debug/tool --help → ヘルプが表示される
[テスト実行コマンド]                 # 例: npm test / cargo test → 0 件でも green で完走すること
```
...
[書き込み系ユースケース (HTTP エンドポイント / CLI コマンドを問わない) のうち、複数テーブルへの書き込みを伴うものについて、単一トランザクションで括る範囲を明記する。DDD モデリングで特定した集約境界 (トランザクション境界と一致させる) をここに書き出す。**本節は「エンドポイント一覧」が「該当なし」(cli モード等) でも省略しない**: cli モードでは 7 章「CLI インターフェース仕様」のコマンド体系から書き込み系コマンドを拾う]

| ユースケース (書き込み系) | 対応 API / コマンド | 書き込み先テーブル      | 境界 (単一 tx / 最終的整合性) | 根拠となる不変条件                                                       |
| -------------------------- | --------------------- | ------------------------ | ------------------------------- | -------------------------------------------------------------------------- |
| [例: 注文確定]              | POST /api/orders      | orders, order_items      | 単一 tx                          | 注文と明細は必ず同時に確定する                                             |
| [例: 一括インポート]        | `tool import`          | accounts, transactions   | 最終的整合性                     | インポート失敗時は取込済み分だけ確定し、失敗分は再実行可能な形で補償する   |

書き込み系ユースケースが一切無い場合のみ「該当なし (理由)」と明記する。集約をまたぐ書き込みを最終的整合性で許容する場合は、その理由 (補償処理・イベント駆動での後追い整合等) を「根拠となる不変条件」列に併記する。
...
[主要シナリオごとに sequenceDiagram。DTO ↔ Domain ↔ Persistence の変換ポイントを含める。複数書き込みを伴うシナリオでは tx 開始 / コミット / ロールバックを図中に明記する]
...
- 途中失敗時のロールバック / 補償: [tx 内は自動ロールバック。tx 外リソース (外部 API・ファイル書き込み等) を巻き込む場合は補償処理の方針]

### ログ出力方針 (アプリ側)

[ログレベル基準 (ERROR / WARN / INFO) と出力フォーマット。収集基盤・アラート閾値は DESIGN_DETAIL_INFRA.md]

## 7. UX 設計 / CLI インターフェース仕様

DESIGN.md のプロダクトモードスタンプで判定する。**webapp モードでは以下の UX 設計、cli モードでは末尾の「CLI インターフェース仕様」に置き換える**。
...
### CLI インターフェース仕様 (cli モードではここに書く)

#### コマンド体系

[サブコマンド一覧。例: `tool init` / `tool run <target>` / `tool status`。階層があれば構造を明示]

#### フラグ/引数表

| フラグ/引数 | 型 | デフォルト | 必須 | 説明 |
| ---------- | -- | ---------- | ---- | ---- |
| `--config`, `-c` | string | `./config.toml` | 任意 | 設定ファイルパス |

#### 入出力形式

- stdout / stderr の使い分け: [結果は stdout、進捗・エラーは stderr 等]
- 出力フォーマット: [テキスト固定 / `--format json` で切替 等]
- パイプ入力対応: [標準入力からの受け取りに対応するか]
...
- `--help` / `-h`: [出力方針。サブコマンドごとに個別ヘルプを持つか]
- エラーメッセージ: [フォーマットと exit code の対応]
...
cli モードの例 (**G_E2E は自動系のみ許可、手動系書式は不可**):

- G1 検証: `cargo test --test cli_init`
- G2 検証: `cargo test --test cli_invalid_args`
- G_E2E 検証: `bash scripts/e2e-cli.sh`
  ([scripts/e2e-cli.sh の中身: ビルド → 各 UC のコマンドを通しで実行 → 各ステップの exit code / stdout・stderr を assert])
```
...
- **機能単体の詳細契約 (出力の正確な形式・エッジケースの決定) は本ファイルに書かず `docs/features/UC-<n>.md` (フェーズ 7.5、`feature-spec.md`) に書く**。本ファイルは横断関心事 (API 一覧・スキーマ・エラー戦略・共通 UX 仕様) のみを持つ。機能仕様が存在しない構成 (クイックモード等) では従来どおり本ファイルに書いてよい
