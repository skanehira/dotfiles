...
feature_spec: |                          # docs/features/UC-<n>.md の全文 (機能スコープなので抜粋しない)。無ければ "null"
  <全文をそのまま>
feature_spec_path: <docs/features/UC-<n>.md または null>
usecase_detail: |                        # USECASES.md の該当 UC 節 (## UC-<n>: から次の ## の直前まで) の全文。BR 表を含む
  <全文をそのまま>
usecases_path: <docs/USECASES.md または null>   # ファイル自体が無い構成 (クイックモード等) では null
...
- `usecase_id` / `feature_spec` / `usecase_detail`: `usecase_id` は TODO.md のフェーズ見出し (`### フェーズ<識別子>:` を PHASE_ID で特定) の `<!-- ucs: (UC-[0-9]+|none) -->` から取る。`UC-<n>` なら `docs/features/UC-<n>.md` の全文を `feature_spec` に、USECASES.md の該当 UC 節 (`## UC-<n>:` から次の `## ` の直前まで) を `usecase_detail` に入れる。**どちらも抜粋せず全文** — 機能スコープなので設計抜粋の 4KB 上限は適用しない。振る舞いの規則 (BR) は USECASES.md 側に住むため、これを渡さないと implementer に BR の本文が届く経路が無い (実測: 過去 run は規定外で同等のキーを improvise していた。本キーはその規定への昇格)
