...
- `prev_phase_summary`: decisions.jsonl の直前フェーズの `event_type: impl_done` エントリ summary を引く。**並列モード (wave サイズ 2 フェーズ以上) では wave 内の他フェーズはまだ実装されていないため参照できない**。wave 開始時点で統合済みの最後のフェーズの summary を入れる (詳細は [parallel-execution.md](./parallel-execution.md) の `### 4p.2`)
