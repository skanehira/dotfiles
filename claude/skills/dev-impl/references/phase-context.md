...
# --- 機能仕様 (UC 単位の詳細設計。dev-spec フェーズ 7.5 の成果物) ---
# 5 キーとも値が無い場合は `<キー>: null` の 1 行で書く (キー自体は常に書く。
# 必須キー照合 14 件の前提で、欠落と「無い構成」を区別するため)
usecase_id: <UC-<n> または null>          # TODO.md のフェーズ見出しの <!-- ucs: ... --> から。none / 宣言なし / TODO.md なし は null
feature_spec: |                          # docs/features/UC-<n>.md の全文 (機能スコープなので抜粋しない)
  <全文をそのまま>
feature_spec_path: <docs/features/UC-<n>.md または null>   # implementer が更新確認や引用元の提示に使う実パス
usecase_detail: |                        # USECASES.md の該当 UC 節 (## UC-<n>: から次の ## の直前まで) の全文。BR 表を含む
  <全文をそのまま>
usecases_path: <docs/USECASES.md または null>   # 抜粋 (usecase_detail) で足りない場合に subagent が自分で Read するための path

# --- このフェーズが踏む攻撃面 ---
risk_faces: [<async_roundtrip|persistence_limit|auth_error_path|ui_consistency|consumable>, ...]
  # Step 4.1 で SPEC_TEXT (issue 本文 + 機能仕様 + UC 節) から算出した集合 (空配列もありうる)。
...
- `phase_tasks`: `gh issue view <N> --json body -q .body` の出力から `## ゴール` / `## DoD` / `## 非スコープ` / `## 実装タスク` の 4 節を切り出す (`## 参照すべき docs` は設計の該当節を読む入口なので、必要な節だけ docs から読んで `design_detail` に入れる。issue の `## 設計` 節は人間向けスナップショットなので抽出しない — 実装器の設計入力は `feature_spec` キーが docs 直読で持つ)
...
- `usecase_id` / `feature_spec` / `usecase_detail`: `usecase_id` は TODO.md のフェーズ見出し (`### フェーズ<識別子>:` を PHASE_ID で特定) の `<!-- ucs: (UC-[0-9]+|none) -->` から取る。`UC-<n>` なら `docs/features/UC-<n>.md` の全文を `feature_spec` に、USECASES.md の該当 UC 節 (`## UC-<n>:` から次の `## ` の直前まで) を `usecase_detail` に入れる。**参照先のファイルが無ければ該当キーは `null`** (旧構成・クイックモード)。**どちらも抜粋せず全文** — 機能スコープなので設計抜粋の 4KB 上限は適用しない。振る舞いの規則 (BR) は USECASES.md 側に住むため、これを渡さないと implementer に BR の本文が届く経路が無い
