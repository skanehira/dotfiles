...
**面の集合 `RISK_FACES` は 2 段で決まる。** 一次は実装前に仕様の日本語から出し (`## 4.1: フェーズ変数の確定と start イベント` のリスク面の一次算出)、二次はこの表のコード差分パターンで補強する。**この表が面の集合の SSOT** — 面を足すときは `claude/skills/dev-spec/references/feature-spec.md` のエッジケース表 (先頭 5 行)・4.1 一次算出の語彙・phase-context.md の `risk_faces` の値一覧にも同時に足す (設計時の問いと実装時の検知を揃えるため)。**二次は言語依存なので、
