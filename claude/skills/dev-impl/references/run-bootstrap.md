...
- [Step 0: カウンタ・予算・gating_decided の復元](#step-0-カウンタ・予算・gating_decided-の復元)
...
- [Step 1: 前提ファイル不在時の挙動](#step-1-前提ファイル不在時の挙動)
- [Step 1: 構造ゲートの判定表](#step-1-構造ゲートの判定表)
- [Step 1.5: PoC マーカー検出結果の分岐](#step-15-poc-マーカー検出結果の分岐)
...
export DOCS_DIR="$(git rev-parse --show-toplevel)/docs"  # traces_to の照合とリスク面の一次算出が読む
...
SKILL.md 「不在時の挙動」の判定表。いずれもエスカレ停止する。

| 不在ファイル                                  | 対処                                                                                                                     |
| --------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------ |
| TODO.md                                       | エスカレ停止: 「TODO.md が無い。`/dev-spec` (フェーズ 10) で生成してから再実行」とユーザー通知                           |
| DESIGN_DETAIL_APP.md / DESIGN_DETAIL_INFRA.md | エスカレ停止: SKILL.md「入力」節のフォールバック案内でユーザー通知 (旧形式の単一 `docs/DESIGN_DETAIL.md` しか無い場合も同じ) |
| DESIGN.md                                     | エスカレ停止: 「DESIGN.md が無い。`/dev-spec` で生成」とユーザー通知                                                     |

## Step 1: 構造ゲートの判定表

ファイルが揃っていても以下に欠けがあれば実装に入らずエスカレ停止する。全フェーズ実装後に発覚しても手遅れなので、起動時に機械判定する。

| チェック                | 判定                                                                                                    | 欠落時の reason / 対処                                                                                                                                                                            |
| ----------------------- | ------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 承認スタンプ            | TODO.md 先頭に `<!-- dev-spec:approved` がある                                                          | `design_not_approved`: 「dev-spec フェーズ 11 の承認ゲートを通してから再実行」                                                                                                                    |
| 承認ハッシュ            | スタンプの `goals_sha=<値>` と再計算値 (SKILL.md Step 1 の GOALS_SHA 算出ブロック) が一致する                                                | `approval_stale`: 「承認後に受入基準 (ゴール / 検証手順) が変更されている。dev-spec フェーズ 9 → 11 で再承認してから再実行」。スタンプに `goals_sha=` が無い旧形式は警告ログのみで通過 (後方互換) |
| ゴール定義              | 本ファイルの `## Step 1: ゴール行の抽出と 1:1 照合`が 1 件以上を返す                                                          | `goals_missing`: 「dev-spec フェーズ 9 でゴールを定義してから再実行」                                                                                                                             |
...
## Step 1.5: PoC マーカー検出結果の分岐

`rg -n '<!-- POC_NEEDED: .* -->' docs/DESIGN.md docs/DESIGN_DETAIL_APP.md docs/DESIGN_DETAIL_INFRA.md` の結果で分岐する。
