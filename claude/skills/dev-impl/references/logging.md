...
**あわせて `run_spawns_budget` (SKILL.md Step 1 で確定した `run_spawns` の上限) を入れる。** フェーズ単位の `start` (再入時の書き直しを含む) には **`phase_spawns_budget`** (既定 33) と **`phase_fix_budget`** (基底は SKILL.md「カウンタと予算」の表が正。`RISK_FACES` の有無で 3 / 4) も入れる。いずれも再入時は引き上げ後の値を書き、復元は記録値の最大を採る。 この値と `phase_added` の同名フィールドだけが予算の記録先で、再入時の復元は両者に記録された値の**最大**を採る (予算は上方向にしか動かないため一意に決まる)。記録が無いと、compaction や再入をまたいだ時点で上限を「記憶」で判断することになる。
