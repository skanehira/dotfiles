...
- [XSS 対策 (生成時注意)](#xss-対策-生成時注意)
- [生成プロセス (dev-impl 視点)](#生成プロセス-dev-impl-視点)
...
<main class="max-w-5xl mx-auto p-8 space-y-8">
  ...
  <!-- Section 4.6: Implementation Notes - Design Decisions -->
  <!-- Section 4.7: Implementation Notes - Open Questions -->
...
## セクション 4.6: 実装ノート — 設計判断

JSONL から `event_type: design_decision` のエントリをフェーズ順に抽出して表示する (Anthropic の implementation-notes プロンプトの「デザインの決定」「トレードオフ」に対応)。カード様式はセクション 4 (P1/P2/P3) を踏襲する。0 件ならセクションごと省略する:

```html
<section class="space-y-2">
  <h2 class="text-lg font-semibold">実装ノート — 設計判断</h2>
  ${designDecisionEvents.map(e => `
    <article class="rounded border p-4" style="border-color: var(--border); background: var(--note-bg); color: var(--note-fg);">
      <header class="flex items-center justify-between text-sm">
        <span class="font-mono">${e.timestamp} / ${e.phase}</span>
        <span class="px-2 py-0.5 rounded text-xs uppercase font-semibold">${e.context.spec_gap}</span>
      </header>
      <p class="mt-2">${escape(e.context.decision)}</p>
      <dl class="mt-3 grid grid-cols-[auto_1fr] gap-x-3 gap-y-1 text-xs">
        <dt>対象</dt><dd>${e.context.affected_files.join(', ')}</dd>
        <dt>関連設計</dt><dd>${e.context.related_design_section ? `<a href="../${e.context.related_design_section}">${e.context.related_design_section}</a>` : 'なし'}</dd>
        <dt>判断根拠</dt><dd>${escape(e.context.rationale)}</dd>
      </dl>
      ${e.context.alternatives.length > 0 ? `
      <details class="mt-2">
        <summary class="cursor-pointer text-xs">検討した代替案 (${e.context.alternatives.length})</summary>
        <ul class="mt-1 text-xs space-y-1 list-disc list-inside">
          ${e.context.alternatives.map(a => `<li>${escape(a.option)} — ${escape(a.rejected_because)}</li>`).join('')}
        </ul>
      </details>` : ''}
    </article>
  `).join('')}
...
## セクション 4.7: 実装ノート — 未解決の質問

JSONL から `event_type: open_question` のエントリを抽出して表示する (Anthropic の implementation-notes プロンプトの「未解決の質問」に対応)。**0 件でも「未解決の質問: なし」を明示表示する** (エンジニアが「確認事項ゼロ」を確信できるように、セクション自体は省略しない):

```html
<section class="space-y-2">
  <h2 class="text-lg font-semibold">実装ノート — 未解決の質問</h2>
  ${openQuestionEvents.length === 0 ? `<p class="text-sm" style="color: var(--fg-muted);">未解決の質問: なし</p>` : openQuestionEvents.map(e => `
    <article class="rounded border p-4" style="border-color: var(--border);">
      <header class="text-sm font-mono" style="color: var(--fg-muted);">${e.timestamp} / ${e.phase}</header>
      <p class="mt-2 font-medium">${escape(e.context.question)}</p>
      <dl class="mt-3 grid grid-cols-[auto_1fr] gap-x-3 gap-y-1 text-xs">
        <dt>背景</dt><dd>${escape(e.context.background)}</dd>
        <dt>確認後の対応</dt><dd>${escape(e.context.suggested_action)}</dd>
        <dt>対象</dt><dd>${e.context.affected_files.join(', ')}</dd>
      </dl>
    </article>
  `).join('')}
...
## セクション 5: POC_NEEDED 残存状況

PoC の解決は前段 dev-spec のフェーズ 5 で完了している前提のため、ここは Step 1.5 の残存ガードが記録した未検証マーカー (`blocker=false` の pending) の一覧を出す。JSONL から `event_type: poc_pending` のエントリだけ抽出し、0 件ならセクションごと省略する:

```html
<section class="space-y-2">
  <h2 class="text-lg font-semibold">POC_NEEDED 残存状況 (pending non-blocker)</h2>
  ${pocs.map(p => `
    <article class="rounded border p-4" style="border-color: var(--border); background: var(--poc-bg); color: var(--poc-fg);">
      <header class="flex items-center gap-3 text-sm">
        ...
        <span class="px-2 py-0.5 rounded bg-white text-xs">risk: ${p.risk}</span>
      </header>
      <p class="mt-1 text-sm">${p.scope}</p>
      <p class="mt-2 text-xs">未検証のまま実装を継続 (blocker=false)。必要になったら tech-investigation を個別に呼ぶ</p>
    </article>
  `).join('')}
</section>
```
...
JSONL から取り出した値 (`rationale` / `diff_before` / `actual_output` / `decision` / `question` / `background` / `suggested_action` 等) は HTML に埋め込む前に**必ず escape する**:
...
2. entries を event_type で分類 (phases / decisions / review_low / design_decisions / open_questions / pocs / goals)。JSONL 記録時点で重複排除済みのため、`design_decision_count` / `open_question_count` は分類後の配列の件数 (`.length`) をそのまま使う
