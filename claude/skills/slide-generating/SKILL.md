...
コンテンツの内容を把握したら、構築前に**互いに異なる 4 つのビジュアル方向**を提案し、ユーザーに 1 つ選ばせる（モデルのデフォルトスタイルへの収束を防ぎ、実行ごとに意味のある多様性を出すため）。

スライドの内容・テーマ・対象者を踏まえて、各方向を以下の形式の表で提示する：

| 方向 | トーン (空気感) | 背景 hex | アクセント hex | フォントペア | 一言理由 |

4 方向は背景色系統・フォント分類（serif / sans / display）・空気感が互いに重ならないようにする。あわせて以下も提示する：

- **差別化**: このスライドを見た人が一番覚えているべきこと
- **参照イメージ**: コンテンツに合うブランド、Webサイト、雑誌のテイスト等

ユーザーが 1 方向を選択（または修正指示）したら、**選ばれた方向のみ**について以下を導出する:
...
スキル同梱の `slide-to-pdf.sh` を実行する。`playwright-core`は共有キャッシュ（`~/.cache/slide-pdf/`）に自動インストールされるため、プロジェクトの`node_modules`を汚染しない。
...
- **関係性を見せる** → diagram
- **目次を作る** → toc
- **会社概要を見せる** → company-profile
- **ファネル/じょうろを見せる** → funnel
- **グラフを描く（CSS）** → chart
- **ランキングを見せる** → ranking
- **数式・演算関係を示す** → formula
- **導入事例を紹介する** → case-study
