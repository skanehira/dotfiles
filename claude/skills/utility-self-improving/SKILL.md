...
なぜ 7 日か: 直近の指摘だけを対象にして 1 回の PR を小さく保つため (ガードレールの「1 PR あたり最大 5 件」と同じ意図)。観測閾値 (3 セッション以上) を満たすかどうかは履歴の濃さ次第なので、引数で `14` や `30` に広げてよい。
...
extractor / judge は fresh context の subagent なので、実行中の細かい進捗は main セッションの画面に出ない。これを補うため、起動形態にかかわらず **`~/.claude/logs/self-improving-progress.log`** に主要マイルストーンを追記する。`claude -p` でヘッドレス実行する場合は `stdout` が完了時に 1 度しか書かれないため、このログが実行中の唯一の可視情報になる。
...
これは最初の数回 (スキルの挙動を確かめる段階) で使う。挙動を確認できたら省略してよい。
