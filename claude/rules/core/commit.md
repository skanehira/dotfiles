...
**この例外の下では、後述「コミットの原則」の 2 項目もフェーズ単位に読み替える**: 「作業中のコードをコミットしない」は「フェーズ範囲のテストが緑でないコードをコミットしない」、「各コミットは機能を壊さずに元に戻せること」は「フェーズ最終コミットまでの列を単位として元に戻せること」(巻き戻し先は `PHASE_START_SHA` かフェーズ最終コミット)。読み替えないと、原則側だけを読んだ第三者やレビュー agent がフェーズ内コミットを違反と判定してしまう。
