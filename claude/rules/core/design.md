...
- 自分の変更で生まれた未使用のメソッド・フィールド・パラメータは即削除する（既存の dead code には触れない。`implementation.md` の外科的変更ルール参照）
...
- 1 ユースケースの不変条件を守る複数書き込みは単一トランザクションで括る (Unit of Work)。境界は docs/DESIGN.md「データスキーマ」内の「トランザクション境界」の表に従う
