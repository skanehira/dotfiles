...
### テストピラミッド: E2E は薄く、単体・結合を厚く

- **単体・結合テスト** で詳細ロジックを網羅 (IO は DI で差し替えて高速実行)
- **E2E は最小限**: 壊れたら業務に致命傷の golden path だけ。実行・保守コストが高いので増やさない
- E2E はリファクタリングや UI 変更で脆くなる。デグレ防止は E2E ではなく単体・結合で担保
- 単体・結合で代替できる E2E は積極的に削除する

### シナリオ不足を能動検出する

ユーザから「シナリオが足りない」と指摘される時点で遅い。実装が一段落したら以下を自発実行:

1. 現在の実装からユーザシナリオを書き起こす
2. 既存テスト (E2E / 単体 / 結合) のカバレッジマップを作る
3. シナリオ × カバレッジ表で漏れを明示
4. 漏れを Critical / Major / Minor で優先度付けして提示

### テスト改善は方針を先に立てる

「何を厚く、何を薄くするか」「削除候補・追加候補のリスト」を文書化してから実装に着手する。
