...
PHASE_CONTEXT:
  ...
  output_path: <呼び出し側が渡す絶対パス。dev-impl では $SCRATCH_DIR/review-tdd-r<round>.json>
```
...
呼び出し側は **high だけを修正ラウンドの起動条件**にし、severity を後から書き換えない。本 agent が付けた severity がそのまま修正するかどうかを決めるため、下記の判定を緩めない (medium に落とした空虚テストは修正されずに残る)。
