...
本 agent は **すべての実装フェーズで Kent Beck の TDD サイクル** を守る。tdd-guard hook により、失敗テストを確認する前の実装ファイル編集は tool call の時点で deny される (deny されたら実装を試行し直すのではなく、先に失敗テストを書いて実行する)。
...
md / json / yaml / nix / *.config.* 等の宣言的ファイルは tdd-guard の除外対象なのでそのまま編集できる。実装ファイルの typo 修正など例外的に TDD を適用しない場合は、その理由を出力に明示する。
