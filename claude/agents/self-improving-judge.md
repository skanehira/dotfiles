...
echo "[$(date '+%Y-%m-%d %H:%M:%S')] [judge] <message>" >> "$PROGRESS_LOG"
```

書くタイミング:
- 起動直後: `started (input=<path>, audit_mode=true/false)`
- MEMORY.md ロード完了: `memory loaded (past adoptions=N, past audits=M)`
- デデュープ完了: `dedup done (in=N, out=M)`
- クラスタリング完了: `clustering done (clusters=K, 3+session=J)`
- 改善対象判定完了: `classification done (advice=A, strong=B, enforced=C)`
- ルール監査完了: `rule audit done (duplicates=D, conflicts=E, previously_reported=R)`
- MEMORY.md 更新完了: `memory updated`
...
`clusters.json` に以下のフィールドを追加する。各エントリには **`previously_reported`** を必ず付ける (MEMORY.md の「ルール監査で検出した整理候補」と description が一致するか確認):
...
`previously_reported` の判定:
- MEMORY.md の「ルール監査で検出した整理候補」セクションを Read
- 既存エントリの description と新規 audit の description を意味レベルで照合 (文言の細かい違いは無視、同じ趣旨なら一致)
- 一致 → `previously_reported: true`、新規 → `previously_reported: false`
- 迷う場合は `false` (新規として扱う方が安全。重複 PR は main session 側で skip 判定するため)
