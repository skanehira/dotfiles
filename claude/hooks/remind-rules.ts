...
5. 監査判定 — 判定するのは「実行するか否か」ではなく「どの観点が必要か」。行数は使わない。原則として /utility-doc-audit を変更文脈つきで呼び、起動する観点は同スキルのトリアージ表に決めさせる。概念の追加・削除・置換 / 節構成の変更 / 新規文書の作成はフル監査。トリアージ表のどの観点にも当たらない改訂 (typo・表記揺れ) だけ「監査スキップ: <変更の性質>」を 1 行出力してスキップしてよい (明示的な監査依頼は基準に関わらず実行)
