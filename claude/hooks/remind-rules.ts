...
// TDD の手順を強制する機械ゲートは無く、順序は自律遵守 + 事後レビューで担保する。
// ここではその運用 (トリアージ / rules の遅延 Read / 外科的変更 / TDD 例外の明示 /
// 実装後のレビュー) をリマインドする。rules/core/*.md は CLAUDE.md から
// 即時展開されない (遅延参照) ため、この Read 指示が実質の読込トリガー。
export const IMPL_REMINDER_FULL =
  `[hook: remind-rules] 実装/修正系の指示を検知。
1. トリアージ: 着手前にタスクを分解し、ユニットごとのモデル割当 (~/.claude/rules/core/orchestration.md の 3 軸判定: 難易度/コンテキスト連続性/並列性) を 1 行で出力する。セッションが最上位 tier (Fable/Mythos 級) で実装作業が支配的なら、着手前に /model opus への切替をユーザに提案する。
...
5. 監査判定 — 判定するのは「実行するか否か」ではなく「どの観点が必要か」。行数は使わない。原則として /utility-doc-audit を変更文脈つきで呼び、起動する観点は同スキルのトリアージ表 (変更の性質 → 観点 1〜5) に決めさせる。概念の追加・削除・置換 / 節構成の変更 / 新規文書の作成はフル監査。トリアージ表のどの観点にも当たらない改訂 (typo・表記揺れ) だけ「監査スキップ: <変更の性質>」を 1 行出力してスキップしてよい (明示的な監査依頼は基準に関わらず実行)
