...
| **受け入れ基準** | 終わったと言える条件。先頭に満たすストーリーの番号を付ける | （US-001）ログイン済みの状態でSQLを送信すると、改善案が3つ以上表示される |
...
受け入れ基準は「どんなときに、何をすると、どうなるか」を 1 文で書き、チェックリストに並べる。各行の先頭には、その基準が満たすストーリーの番号を `（US-<番号>）` の形で付ける。チェックボックスは一覧の書式で、印は付けない。ユースケースの完了は、そのユースケースの親 issue の子 issue がすべて close したことで判定する。

前提条件・操作・期待結果を別々の行に分けない。3 つの要素が 1 文に収まらないときは、ユースケースかストーリーが大きすぎる合図なので分割を検討する。

### 例

[usecase-template.md](usecase-template.md) の記述例（UC-001: SQLを分析して改善案を得る）の受け入れ基準。

```markdown
### 受け入れ基準
- [ ] （US-001）ログイン済みでプロジェクトを選んだ状態で、SQLとスキーマを入力して「分析」を押すと、改善案が3つ以上一覧表示される
- [ ] （US-001）スキーマを入力する段階で、「インポート」からDDLファイルを選ぶと、パースしたスキーマが入力欄に反映される
- [ ] （US-001）SQLを入力する段階で、11件以上のクエリを入れて「分析」を押すと、上限が10クエリであることが表示されて分析は始まらない
- [ ] （US-002）「分析」を押したあとで、AIエンジンが30秒以内に応答しないと、処理中であることが表示され、完了後にメールで通知が届く
```
...
| 要素         | 例の 1 行目の該当箇所                          |
| ------------ | ---------------------------------------------- |
| どんなときに | ログイン済みでプロジェクトを選んだ状態で       |
| 何をすると   | SQLとスキーマを入力して「分析」を押すと        |
| どうなるか   | 改善案が3つ以上一覧表示される                  |

「SQLを入力できる」のように操作だけを書いた項目は、どうなれば成功かが読めないので不合格。期待結果まで書く。

### フローとビジネスルールからの起こし方

受け入れ基準は新しく考えるのではなく、同じユースケースのフローとビジネスルールから起こす。下の表の例は、上の例の各行がどこから来たかを示す。

| 起こす元 | 受け入れ基準の「どうなるか」にするもの | 例 |
| -------- | -------------------------------------- | -- |
| 正常系フロー | 最後のステップで利用者に見える結果 | 1 行目（正常系のステップ6「改善案を一覧表示する」） |
| 代替フロー | 分岐を選んだときの結果 | 2 行目（代替フロー A のステップ2d） |
| 例外フロー（E1, E2 …） | 1 つの例外フローにつき 1 行 | 4 行目（例外フロー E1） |
| ビジネスルール | 上限・下限・区分の境界で起きること | 3 行目（BR-001「最大10クエリまで」） |

フローにも BR にも無い基準を書きたくなったら、フローか BR の方が欠けている。先にそちらへ足してから基準を書く。

### 接頭辞の規則

- 括弧は全角の `（）` を使い、番号は USER_STORIES.md の見出し `#### US-<番号>` と同じ表記にする
