...
担う受け入れ基準: <参照する機能設計書「テスト方針」の表のうち、この issue が満たす行を文面そのままで列挙する。
 1 つの機能設計書の表を複数の子で分けるときだけ書く。この子が機能設計書の表を全行担うときは行ごと省く>
...
<横断事項 (スキーマ・API 規約・認証など) に触れる場合は「docs/design/DESIGN.md「<節名>」」も併記。設計本文は転記しない — 正本は docs 側で、issue は参照のみ。
 DESIGN.md「既知の制約」に、この issue が触れる範囲の制約 (クエリ上限・ID の順序・時刻の基準など) があれば必ず併記する。
 実装者は issue が名指しした docs しか読まないため、併記しなければ制約は実装に届かない>
 ...
 **書く基準は「この issue の DoD 実行に、依存先が merge した成果物が無ければ失敗するか」だけ。**
 同じ機能に属する・依存先を参照する・順に作ると自然、という理由では書かない (下記「依存の段数」)。
 **フェーズ 9 のドラフトでは GitHub の番号がまだ無いので `Depends on issue-<連番>` と書き**
 (連番はドラフトのファイル名 `issue-<連番>.md` のもの)、フェーズ 10 の作成時に `#<番号>` へ置換する>
```
...
## 受け入れ基準
docs/design/USECASES.md「<UC の見出し>」の受け入れ基準を参照。検証は子 issue の DoD と、機能設計書「テスト方針」が割り当てたテストで行う。本 issue には転記しない。
...
| 親 | タイトル | 立てる条件 |
| --- | --- | --- |
| 基盤 | `基盤: UC に属さない共通実装` (固定文字列) | どの UC にも属さない子 (スキーマ整備・監査ログ基盤・CI 整備など) が 1 件以上ある。「ユースケース」節と「受け入れ基準」節は `該当なし (UC に属さない共通実装)` と書く |
| UC 統合 | `<UC 番号 1> / <UC 番号 2>: <見出し本文 1> と <見出し本文 2>` (例: `UC-004 / UC-005: 打刻する と 会場間を移動する`) | 2 つの UC が密結合で、子を分けても実装・検証の単位が分かれない。「ユースケース」節と「受け入れ基準」節に両方の見出しを列挙する。**統合は 2 件までとし、3 件以上は統合せず親を分ける** (子側の「対応 UC」行で複数列挙する) |
...
UC が存在しないため、親は `<タスク名> トラッキング` 1 件にフォールバックし、全子をそこに紐付ける。本文の「ユースケース」節と「受け入れ基準」節は省き、「設計」節に DESIGN.md を書く。子の「対応 UC」行は `基盤` と書き、対応表では `基盤` キーをこの親の番号に seed する。
...
3. **依存の整合と段数**: 依存グラフに循環が無いか。「依存なし」の issue が実は他 issue の成果物を前提にしていないか。加えて**依存の段数**を検査する (下記「依存の段数」の基準と算出手順に従い、`依存段数:` の 1 行を findings の前に必ず出す)
4. **相互の重複・漏れ**: 同じ作業が複数 issue に跨っていないか。機能設計書の「実装の配置」に現れるファイルがどの issue にも属さず漏れていないか。機能設計書「テスト方針」の表の各行が、ちょうど 1 つの子に担われているか (「担う受け入れ基準」行を持つ子はその行、持たない子は参照する機能設計書の全行を担う)
...
フェーズ 9 は上記 6 項目を検査 subagent の指示文に転記するので、**例外の但し書き (項目 6 の末尾) と、下記「依存の段数」の基準・算出手順・閾値も一緒に転記する** — 転記しないと検査側は例外表も段数の基準も見ておらず、基盤親・統合親に対して必ず high を上げ、段数の行も出せない。

## 依存の段数

`/dev-impl` は同じ依存レベルの issue を最大 3 件まで並列に実装する。並列度を決めるのは issue の件数ではなく**依存の段数**で、run の所要時間は「段ごとに最も長い 1 件を待つ」形で決まる (実測: セッション 82fc5ce1 で子 22 件を 10 段に割った run では、段ごとの最長サイクルの和 375 分に対し実経過が 386 分だった)。段を浅く割ることが、実装ループの所要時間に最も効く。

**`Depends on` を書く基準**: その issue の DoD 実行に、依存先が merge した成果物が無ければ失敗する場合だけ書く。書かない例:

- 依存先が作る型やヘルパーを**参照するだけ**で、この issue の DoD が通る (参照先を自分で用意できる、またはインタフェースが docs で確定している)
- 同じ機能設計書に属する / 画面と API が対になっている、という理由だけの並び
- 順に作ると人間には自然、という理由だけの並び

**算出手順**: 依存が無い issue を L0、それ以外は「依存先のレベルの最大値 + 1」とする (`/dev-impl` が着手順の判定に使う定義と同一)。GitHub の issue 番号はドラフト段階でまだ無いので、**ドラフトの連番 (`issue-<連番>.md`) と `Depends on` の対応でレベルを計算する**。

**報告と閾値**: findings の前に次の 1 行を必ず出す。

```
依存段数: L0..L<k> (<k+1> 段、実行バッチ <b> / 下限 <m>)
```

**比較するのは段数ではなく実行バッチ数**である。段数 (`<k+1>`) は依存の深さ、`<b>` は「各段を 3 件ずつに分けて流したときの総バッチ数」= `Σ ceil(|L_i| / 3)` (`|L_i|` は段 i に属する子の件数)、`<m>` は下限 `ceil(子 issue 件数 / 3)`。段数と下限を直接比べると単位が合わず、依存を削って浅くするほど「1 段、下限 8 段」のような矛盾した行が出る (子 22 件を全て L0 に置いた場合、段は 1 でも 3 並列なら 8 バッチ要る)。

`<b>` が `<m>` を超える場合は `[medium]` の finding として、**削れる可能性のある `Depends on` 行を列挙する** (どれを削るかは設計判断なので、指摘に留めて修正はしない)。超過分 `<b> - <m>` が、依存を浅く割れば取り戻せるバッチ数である。

この比較が意味を持つのは **`/dev-impl` の並列前提 (dev-impl SKILL.md 2.0 の 2 条件: worktree セットアップ手順と固定ポート非共有) を満たすプロジェクトだけ**である。満たさない repo は並列度 1 で回り、段数を減らしても所要時間は縮まらない。その場合は比較を行わず、行に「(並列前提を満たさないため閾値判定なし)」と添える。
...
**作成の直前に、ドラフト本文の `Depends on issue-<連番>` を `Depends on #<番号>` へ置換する** (連番 → その連番のドラフトのタイトル → 対応表の番号、の順に引く)。上流から順に作るので、置換に要る番号は常に確定済みである。置換し残しがないことを `rg 'Depends on issue-' <scratchpad>/issue-*.md` が 0 件になることで確かめる (残すと dev-impl が着手順を解決できない)。
