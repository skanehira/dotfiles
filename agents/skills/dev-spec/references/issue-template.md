...
`/dev-impl` は同じ依存レベルの issue を最大 3 件まで並列に実装する。並列度を決めるのは issue の件数ではなく**依存の段数**で、run の所要時間は「段ごとに最も長い 1 件を待つ」形で決まる (実測: 子 22 件を 10 段に割った run では、段ごとの最長サイクルの和 375 分に対し実経過が 386 分だった)。段を浅く割ることが、実装ループの所要時間に最も効く。
...
**報告と閾値**: findings の前に `依存段数: L0..L<k> (<k+1> 段、理論下限 <m> 段)` の 1 行を必ず出す。理論下限 `<m>` は `ceil(子 issue 件数 / 3)` — 並列枠 3 を使い切ったときの段数の下限である。実段数がこれを超える場合は `[medium]` の finding として、**削れる可能性のある `Depends on` 行を列挙する** (どれを削るかは設計判断なので、指摘に留めて修正はしない)。
