...
ユーザーから議題と必要なコンテキスト（関連ファイル、前提条件、ゴール）を受け取る。不明確な場合は `{{@ask-user}}` で確認する。
...
`{{@ask-user}}` で「この構成で議論を開始してよいか / 調整したいか」を確認する。ユーザーがロール変更を求めたら調整してから次へ進む。

### フェーズ3: 初回意見の収集

各ロールに並列で意見を出してもらう。{{@spawn-subagent}}を `subagent_type={{@general-agent}}` で複数同時に起動する（並列性を活かす）。
...
- `/dev-spec` — 議論ではなく設計ループの中で要件を引き出したい場合 (フェーズ 6・7 が決めをその場で質問する)
