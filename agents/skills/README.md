...
| [dev-impl](./dev-impl/) | 実装ループ。GitHub issue を `Depends on #N` の順に 1 件ずつ自律実装 (implementer subagent が issue と docs を直読して TDD → review-impl → high だけ修正 ≤1 ラウンド → 検証 green → PR → merge → close)。詰まった issue は needs-human で駐車して次へ。進捗は issue コメントのみで、修正対象にしなかった medium はチェックリスト (docs/pending-review/issue-<N>.html、リポジトリで持ち回り) に記録して run 終了時に確認を促す | GitHub issue (必須)。docs/design/DESIGN.md + docs/design/features/ は issue から参照される | issue ごとの PR + merge コミット |
