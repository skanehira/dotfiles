...
| **S: バグ修正・typo** | 直接依頼 | スキル不要。グローバル指示 (CLAUDE.md) の「実装時」の rules を自分で Read して守る |
...
| [dev-impl](./dev-impl/) | 実装ループ。GitHub issue を `Depends on #N` の依存順に自律実装 (同じ依存レベルは最大 3 並列、merge は直列。implementer subagent が issue と docs を直読して TDD → review-impl → high だけ修正 ≤1 ラウンド → 検証 green → PR → merge → close)。詰まった issue は needs-human で駐車して次へ。進捗は issue コメントのみで、保留した high (test-quality / code-quality) と medium は issue 別のチェックリスト (docs/pending-review/issue-<N>.html、リポジトリで持ち回り) に記録して run 終了時に確認を促す | GitHub issue (必須)。docs/design/DESIGN.md + docs/design/features/ は issue から参照される | issue ごとの PR + merge コミット |
