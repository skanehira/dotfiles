...
ユーザにしか決められない情報 (ゴールの曖昧さ、動作検証の手段、スコープ境界など) があれば `{{@ask-user}}` で確認する。
...
書き直したプロンプト全文と、「どの observation をなぜ適用したか」の簡潔な要約を通常のテキストでユーザに提示したうえで、末尾に番号付き選択肢を **テキストで** 質問する（`{{@ask-user}}` は使わない。ダイアログ UI がプロンプト全文の表示スペースを圧迫し、ユーザが書き直し内容を確認できなくなるため）。
