...
- **hooks 前提の記述** (`commit-msg-guard` 等による機械強制): Codex には同等の hook 機構がないため、`~/.claude/rules/core/commit.md` を読み、その内容を自分の判断で遵守する
- **`chrome-devtools` MCP**: `codex/config.toml` で設定済みの `chrome-devtools` MCP サーバをそのまま使う (Claude 側と同一パッケージ)
